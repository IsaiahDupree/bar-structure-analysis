%% Create MATLAB Plots matching Python implementation
% This script creates plots identical to the Python implementation and saves them
...
fprintf('Saved enhanced_displacement_field.png\n');
...
for f = 1:3
    ...
    arrow([pos, max(stress_analytical)*0.6], [pos-20, max(stress_analytical)*0.6], 'Color', [1 0 0], 'Width', 2);
...
fprintf('Saved enhanced_stress_field.png\n');
...
hold on;
yline(5, '--', 'Color', [1 0 0], 'LineWidth', 2, 'DisplayName', '5% Error Threshold');
...
for i = 1:length(percent_error)
    if percent_error(i) > 5
        bar(x_element_centers(i), percent_error(i), 0.7, 'FaceColor', [1 0 0], 'EdgeColor', [0.7 0 0]);
    end
end
...
fprintf('Saved enhanced_error.png\n');
...
fprintf('Saved enhanced_area_distribution.png\n');
...
fill(x_seg1, width_seg1_top, [0 0 1], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
fill(x_seg1, width_seg1_bottom, [0 0 1], 'FaceAlpha', 0.3, 'EdgeColor', 'none');

% Segment 2 (constant area)
x_seg2 = linspace(L, 2*L, 100);
width_seg2 = A2/10;
fill([L, 2*L, 2*L, L], [width_seg2, width_seg2, -width_seg2, -width_seg2], [0 0 1], 'FaceAlpha', 0.3, 'EdgeColor', 'none');

% Segment 3 (constant area)
x_seg3 = linspace(2*L, 3*L, 100);
width_seg3 = A3/10;
fill([2*L, 3*L, 3*L, 2*L], [width_seg3, width_seg3, -width_seg3, -width_seg3], [0 0 1], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
...
fprintf('Saved enhanced_combined_visualization.png\n');

%% 6. Generate report in the matlab directory
fprintf('Generating report in the matlab directory...\n');
...
fclose(fid);
fprintf('Saved matlab_bar_analysis_report.txt\n');
...
fclose(fid);
fprintf('Saved matlab_detailed_analysis.md\n');
